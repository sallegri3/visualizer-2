...
class Generate_Graph:
        ...
        for node in self.nx_graph.nodes:
            ...
            if self.unique_id_data_dict[node]['sn_or_tn'] == 'source_node':
                if self.nx_graph.degree(node) != 0:
                    # len(self.source_target_edge_value_dict[node]) != 0
                    # print(self.nx_graph.degree(node))
                    temp_df = self.edges_df[self.edges_df['source_id'] == node]

                    for target in self.nx_graph.neighbors(node):
                        # for target in self.source_target_edge_value_dict[node]
                        # print(self.source_target_edge_value_dict[node][target])
                        # print(self.edges_df[(self.edges_df['source_id'] == node) & (self.edges_df['target_id'] == target)].iloc[0]['edge_value'])
        ...
        for node in selected_nodes_list:
            if node['sn_or_tn'] == 'source_node':
                ...
                for _, connecting_node in enumerate(self.nx_graph[node['id']]):
                    # print(self.source_target_edge_value_dict)
                    ...
                    # edges[str(self.unique_id_data_dict[connecting_node]['name']) + ' (ID:' + str(connecting_node) + ')'] = 'test'
    ...
    def update_graph_elements(self):
        ...
        print(self.graph_update_shortcut)
    ...
    def reset_graph(self):
        ...
        # self.source_target_edge_value_dict = self.source_target_edge_value_dict_initial
    ...
    def load_additional_data(self, df_input):
        ...
        # print('BEFORE GRAPH SIMULATION: ' + str(time.time() - start_time))
    ...
    def _identify_highly_connected_nodes(self): 

        source_edge_series = self.edges_df['source_id']
        target_edge_series = self.edges_df['target_id']
        ...
        self.highly_connected_nodes = clustered_df[clustered_df['cluster_2'] == 0]['nodes'].to_list()
        self.slightly_connected_nodes = clustered_df[clustered_df['cluster_2'] == 1]['nodes'].to_list()
